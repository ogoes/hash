...
std::string Player::insert_into () {
  ...
  std::string query (
      "INSERT INTO Player (id, username, password, wins, tieds, lost) VALUES "
      "(NULL" );
  std::string attr;

  query += std::string ( "," );
  attr = this->has_username ? this->username : std::string ( "NULL" );
  query += attr;

  query += std::string ( "," );
  attr = this->has_password ? this->password : std::string ( "NULL" );
  query += attr;
...
std::string Player::query_builder ( const char* columns = "*" ) {
  ...
  if ( this->has_id || this->has_username ) {
    ...
    where = this->has_id && this->has_username ? std::string ( " AND " )
                                               : std::string ( "" );
    query += where;
...
std::string Player::update () {
  ...
  if ( this->has_username ) {
    set = std::string ( "username = " ) + this->username;

    where = this->has_id
                ? std::string ( " WHERE id = " ) + std::to_string ( this->id ) +
                      std::string ( " AND " )
                : std::string ( " WHERE " );
  ...
  if ( this->has_password ) {
    ...
    set += std::string ( "password = " ) + this->password;
    has_prev_attr = true;
  }
  query += set;
...
std::string Player::delete () {
  ...
  if ( !this->has_id && !this->has_username ) {
    td::cerr << "Operação necessita de uma parâmetro WHERE" << std::endl;
    return NULL;
  }

  if ( this->has_id ) {
    where = std::string ( "id = " ) + std::to_string ( this->id );
  }
  query += where;

  if ( this->has_username ) {
    where = this->has_id ? std::string ( " AND " ) : std::string ( "" );

    where += std::string ( "username = " ) + this->username;
  }
  query += where;
